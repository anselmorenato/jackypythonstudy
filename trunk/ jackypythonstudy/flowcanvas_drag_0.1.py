...
class DataShapeSimple(ogl.RectangleShape):
    ...
    def __init__(self, w=0.0, h=0.0,text =''):
        ...
        self.AddText(text)
class circleshape(ogl.CircleShape,wx.EvtHandler):
    def __init__(self,*args,**kwds):
        ogl.CircleShape.__init__(self,*args,**kwds)
        wx.EvtHandler.__init__(self)
class DataShape(ogl.CompositeShape,wx.Window):
    def __init__(self, canvas, name=''):
        ogl.CompositeShape.__init__(self)
        self.canvas = canvas
        parent =self.canvas.GetParent()
        #self.diagram = self.canvas.GetDiagram()
        self.SetCanvas(self.canvas)       
        wx.EvtHandler.__init__(self)
        #wx.Window.__init__(self,parent)
        #parent.Show(True)
        #self.Bind(wx.EVT_RIGHT_DOWN,self.on_right)
        #wx.EVT_RIGHT_DOWN(self,self.on_right)
        ...
        #self.ProcessEvent(wx.EVT_RIGHT_DOWN)
        
        
        #wx.EVT_LEFT_DOWN(shape2,self.on_shape2_leftdown)
    def on_shape2_leftdown(self,evt):
...
class TaskShape(ogl.CompositeShape,wx.Window):
    """
    ...
    def __init__(self, canvas, name =''):
        ...
        #wx.EvtHandler.__init__(self)
        #wx.Window.__init__(self,canvas)
        ...
        self.middleshape = MiddleShape(100,120,canvas,self.taskobject_name)  # this is the  the constraining shape

        self.inputshape = InputSocketShape(30,120)
        self.inputshape.SetBrush(wx.WHITE_BRUSH)

        self.outputshape = ogl.DrawnShape()
        self.outputshape.SetDrawnBrush(wx.GREEN_BRUSH)
        self.outputshape.DrawPolygon([(0,60),(0,0),(0,-60),(30,0)])

        self.AddChild(self.inputshape)
        self.AddChild(self.middleshape)
        self.AddChild(self.outputshape)

        # If we don't do this, the shapes will be able to move on their
        # own, instead of moving the composite
        self.middleshape.SetDraggable(True)
        self.inputshape.SetDraggable(0)
        self.outputshape.SetDraggable(0)

        # ------------------------------------------------------------
        self.constraint = ogl.Constraint(ogl.CONSTRAINT_LEFT_OF,self.middleshape, [self.inputshape])
        self.constraint.SetSpacing(0.2,0)
        self.constraint2 = ogl.Constraint(ogl.CONSTRAINT_MIDALIGNED_RIGHT,self.middleshape,[self.outputshape])
        self.constraint2.SetSpacing(1,0)
        
        self.AddConstraint(self.constraint)
        self.AddConstraint(self.constraint2)
        self.Recompute()
        # If we don't do this the shape will take all left-clicks for itself
        self.middleshape.SetSensitivityFilter(0)
        self.inputshape.SetSensitivityFilter(0)
        self.outputshape.SetSensitivityFilter(0)
        
        wx.EVT_LEFT_DOWN(self.inputshape,self.on_input_leftdown)
    def _delete(self):
        """
        Fully disconnect this shape from parents, children, the
        canvas, etc.
    ...
    def on_right(self,evt):
        ...
        self.canvas.Refresh()
        evt.Skip()
    def on_input_leftdown(self,evt):
        print 'input shape is select.'
        
class InputSocketShape(ogl.RectangleShape,wx.EvtHandler):
...
class EvtHander(ogl.ShapeEvtHandler,wx.EvtHandler):
    
    def __init__(self,canvas,frame,shape,*args,**kwargs):
        ogl.ShapeEvtHandler.__init__(self)
        
        #self.log = log
        self.frame = frame
        self.canvas = canvas
        #self.shape = self.GetShape()  #noty
        self.shape = shape
        
        #wx.EvtHandler.__init__(self)
        wx.EVT_LEFT_DCLICK(shape,self.on_left_Dclick)
        wx.EVT_RIGHT_DOWN(self.shape,self.on_right_click)
        #wx.EVT_LEFT_DCLICK(self.shape,self.on_left_Dclick)
        #wx.EVT_KEY_DOWN(self.shape,self.on_delete_key)
    ...
    def on_left_Dclick(self,evt):
        print 'double',evt.GetEventObject()
        #self.GetShape()._delete()
    ...
    def OnRightClick(self,x,y,keys = 0, attachment =0): 
        shape = self.GetShape()
        shape.SetDraggable(1)
        #shape = self.GetShape()
        
        #self.frame.make_popmenu()
        #if shape.Haschild
        #if shape == TaskShape(shape.GetCanvas()):
        #self.log.write('this is right ckick',shape)
...
class FlowCanvas(ogl.ShapeCanvas):
    ...
    #def __init__(self, parent,frame):
        ...
        #wx.EVT_RIGHT_DOWN(shape, self.on_right_click)
    ...
    def add_shape(self, shape, x, y, pen, brush, text):
        ...
        shape.Show(True)
        shape =shape
        self.evthandler = EvtHander(canvas = self,frame = self.frame,shape = shape)
        
        
        shape.PushEventHandler(shape)
    ...
    def delete_shape(self):
        ...
        #print shape, shape.id
        shape._delete()
        
        self.Refresh()
        print self.shapes
        #print self.shapes.remove(shape)
    def on_right_click(self,evt):
...
class FlowFrame(wx.Frame):
    def __init__(self,parent,Id =-1,name ="",log=None):
...
def run():
    ...
    ogl.OGLInitialize()
    frame = FlowFrame(None, -1,"", sys.stdout)
