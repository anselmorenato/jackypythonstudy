...
# 
#
# $Header: /export/cvs/python/packages/share1.5/AutoDockTools/Utilities24/summarize_results4.py,v 1.4.2.2 2007/05/14 22:04:08 rhuey Exp $
#
# $Id: summarize_results4.py,v 1.4.2.2 2007/05/14 22:04:08 rhuey Exp $
#
import os, glob
...
def get_largest_cluster_info(d, rms_tolerance, build_hydrogen_bonds=False, \
    ...
    dlo_filename = get_filename(d, c)
    ostr += dlo_filename + ","
    ostr += "%3d,%3d,%3d,% 8.4f,% 8.4f," %(len(d.clusterer.data), num_clusters, len(largest),  c.binding_energy, c.getRMSD())
    if verbose: print "set dlo_filename to ", dlo_filename
...
if __name__ == '__main__':
    ...
    try:
        opt_list, args = getopt.getopt(sys.argv[1:], 'd:o:t:f:r:bLBkauvh')
    ...
    for o, a in opt_list:
        ...
        #this option is not working currently:
        #if o in ('-e', '--e'):
    ...
    if first:
        tstr = ""
        if print_best_only:
            tstr += "lowestEnergy dlgfn     #runs #cl #LEC    LE      rmsd "
            if build_hydrogen_bonds:
                tstr+= " #hb "
            if report_energy_breakdown:
        ...
        if print_Largest_only:
            if print_best_only:
                tstr += "   "
            tstr += " largestCl_dlgfn    #runs #cl#LEC   LE        rmsd "
            if build_hydrogen_bonds:
                tstr+= " #hb "
            if report_energy_breakdown:
                tstr+= " #ESTAT      #HB     #VDW   #DSOLV "
    ...
    if print_best_only or print_Largest_only:
        if print_best_only:
            ...
            ostr += get_best_energy_info(d, rms_tolerance, build_hydrogen_bonds, report_energy_breakdown, report_unbound_energy, receptor_filename)
        if print_Largest_only:
            if verbose: print 'print_Largest_only is True'
            ostr += get_largest_cluster_info(d, rms_tolerance, build_hydrogen_bonds, report_energy_breakdown, report_unbound_energy, receptor_filename)
        ...
        for clust in d.clusterer.clustering_dict[rms_tolerance]:
            ...
            if build_hydrogen_bonds or report_energy_breakdown:
                d.ch.set_conformation(c)
            ostr = "%3d,% 8.4f,% 8.4f" %(len(clust), c.binding_energy, c.getRMSD())
