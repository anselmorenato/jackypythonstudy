...
class CompositeDataShape(ogl.CompositeShape):
    def __init__(self,canvas):
        ...
        constraining_shape = ogl.RectangleShape(100, 60)
...
class MiddleShape(ogl.CompositeShape):
    def __init__(self, canvas):
        ogl.CompositeShape.__init__(self)
        self.SetCanvas(canvas)
        
        shape1 = DividedShape(100,120,canvas)
        shape2 = ogl.DrawnShape()
        shape2.SetDrawnBrush(wx.GREEN_BRUSH)
        shape2.DrawPolygon([(0,60),(0,0),(0,-60),(30,0)])
        ...
        constraint = ogl.Constraint(ogl.CONSTRAINT_MIDALIGNED_RIGHT,shape1, [shape2])
        self.AddConstraint(constraint)
        self.Recompute()
...
class TaskShape(ogl.CompositeShape):
    def __init__(self, canvas):
        ...
        middleshape = DividedShape(100,120,canvas)
...
class FlowFrame(wx.Frame):
        def __init__(self, *args, **kwds):
                ...
                self.canvas = FlowCanvas(self,splitter1)  
                self.Center()
