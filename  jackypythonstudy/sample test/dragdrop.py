...
class DragDrop(wx.Frame):
    def __init__(self, parent, id, title):
        ...
        dt2 = MyTextDropTarget(self.lc1)
        self.lc1.SetDropTarget(dt2)
        self.Bind(wx.EVT_LIST_BEGIN_DRAG, self.OnDragInit, id=self.lc2.GetId())
        
        tree = self.dir.GetTreeCtrl()

        splitter1.SplitVertically(self.dir, splitter2,200)
        splitter2.SplitVertically(self.lc1, self.lc2,300)
    ...
    def OnSelect(self, event):
        try:
            ...
            dlg = wx.MessageDialog(None,'the drivie is not exit','Error!')
            dlg.ShowModal()
            dlg.Destroy()
    def OnDragInit(self, event):
        obj = event.GetEventObject()  #this is added new 
        text = obj.GetItemText(event.GetIndex())  # use 'obj' instead the 'self.lc1'
        tdo = wx.TextDataObject(text)
        tds = wx.DropSource(obj)
        tds.SetData(tdo)
        tds.DoDragDrop(True)
