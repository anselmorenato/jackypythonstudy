...
class TaskLocal(object):
    ...
    def initialize_event(self):
        self.__converted_event = NagaraEvent()
...
class LocalState(object):
    ...
    # event handlers
    @binder('__tasklocal.converted_event')
    def __converted_on_tasklocal(self, msg):
        self.__log_recieve('__converted_on_tasklocal')
        try:
            self.__state.receive_converted_event()
        except AttributeError:
            pass

    @binder('__tasklocal.received_event')
    def __received_on_tasklocal(self, msg):
        self.__log_recieve('__received_on_tasklocal')
        try:
