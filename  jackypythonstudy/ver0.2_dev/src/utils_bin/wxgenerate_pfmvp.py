...
class InterfaceGenerater(object):
    ...
    def parse_config(self, file):
        ...
        self._events = events
        self._properties = properies
        self._getsets = getsets
    ...
    def inject_header(self):
        template = (
            '{indent}#!/usr/bin/env python\n'
            '{indent}#  -*- encoding: utf-8 -*-\n'
            '{indent}# Copyright (C)  2009 Takakazu Ishikura\n'
            '{indent}import os, sys\n'
            '{indent}from abc '
            'import abstractmethod, abstractproperty, ABCMeta\n'
        )
        return template.format(indent='')
    ...
    def inject_enable(self, key):
        template = (
            '{indent}@abstractmethod\n'
            '{indent}def enable_{key}(self): pass\n'
            '{indent}@abstractmethod\n'
            '{indent}def disable_{key}(self): pass\n'
            '{indent}@abstractmethod\n'
            '{indent}def is_{key}_enabled(self): pass\n'
        )
        return template.format(key=key, indent=' '*4)
...
class MockGenerater(object): 
    def __init__(self, is_override=False, use_wxevent=True, use_pubsub=False):
        self.is_override = is_override

        self.use_wxevent = use_wxevent
        if self.use_wxevent:
            self.use_pubsub = False
        else:
            self.use_pubsub = True

        self.event_mode = 'wxEvent'
        self.is_enable = True
    ...
    def inject_class(self):
        ...
        if class_name.endswith('View'):
            ...
            cn = class_name
            obj_name = ''
        interface_name = '{0}_i{1}'.format(cn.lower(), obj_name)
        iclass_name = 'I'+class_name

        if self.use_wxevent:
            template = (
                '{indent}import {interface_name}\n'
                '\n'
                '{indent}class {key}({interface_name}.{iclass_name}, '
                'wx.EvtHandler):\n'
                '{indent}    def __init__(self, log=sys.stdout):\n'
                '{indent}        wx.EvtHandler.__init__(self)\n'
            )
        else:
            template = (
                '{indent}import {interface_name}\n'
    ...
    def inject_event(self, key):
        template = (
            ...
            '{indent}    return self._{key}_event\n'
        )
        return template.format(key=key, indent=' '*4)
    ...
    def inject_enable(self, key):
        template = (
            '{indent}def enable_{key}(self):\n'
            '{indent}    self._{key} = True\n'
            '{indent}def disable_{key}(self):\n'
            '{indent}    self._{key} = False\n'
            '{indent}def is_{key}_enabled(self):\n'
            '{indent}    return self._{key}\n'
        )
        return template.format(key=key, indent=' '*4)

    def inject_send_event(self, key):
        template = (
            '{indent}def _send_{key}(self, event):\n'
            '{indent}    self.{key}_event.fire()\n'
        )
        return template.format(key=key, indent=' '*4)
...
class PresenterGenerater(object):
    ...
    def parse_config(self, file):
        ...
        self._class_name = class_name
        self._output_filename = class_name.lower() + '_p.py'
    ...
    def inject_init(self):
        ...
        codes.append(' '*4 + '# events')
        template = (
            '{indent}def __init__(self, model, view, log=sys.stdout):\n'
            '{indent}    self.log = log\n'
    ...
    def inject_listener(self, key, obj_name):
        template = (
            '{indent}def {key}_on_{obj}(self, event):\n'
            "{indent}    self._log_recieve('{key}_on_{obj}')\n"
        )
        return template.format(key=key, obj=obj_name, indent=' '*4)

    def inject_log_recieve(self):
        template = (
            '{indent}def _log_recieve(self, listener_name):\n'
            '{indent}    cn = self.__class__.__name__\n'
            "{indent}    mes = 'Recieved on {{0}} at {{1}}\\n'\n"
            '{indent}    self.log.write( mes.format(listener_name, cn) )\n'
        )
        return template.format(indent=' '*4)
